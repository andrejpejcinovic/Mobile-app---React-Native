...
const QuizCard = ({
  ...
  const cardScale = () => {
    if (isVertical && !isLandscape) return 1;
      ...
      if (screenCount === 3 && questionIndex === 3) return 0.7;
      else return 0.8;
    }
  };
  ...
  const cardWidth = () => {
    if (isVertical && !isLandscape) return "90%";
      ...
      // da u tom slucaju adaptira sirinu, jer u protivnom izlazi iz
      // granica svoje kartice
      if (screenCount === 3 && questionIndex === 2) return "30%";
      else return "50%";
    }
  };
    ...
    <>
      ...
      >
        <View>
          ...
          <Text style={{ ...styles.text, ...quizCardStyle.questionText }}>
            {question?.text || "Nema pitanja"}
          </Text>
        </View>
